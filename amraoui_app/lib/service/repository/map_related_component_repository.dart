...
Future<List<double>> getCityLatLong({required String city, required int initialLat, required int initialLong})async{
try {
 var response = await AppApi().sendRequest.get("https://maps.googleapis.com/maps/api/geocode/json?address=$city&key=${AppConstMapKey.instance.googleMapApiKey}");
...
if(response.data["results"] != null && response.data["results"] is List){
  ...
  if(result["geometry"] != null && result["geometry"] is Map){
    ...
    if(geometry["location"] != null && geometry["location"] is Map){
      ...
      var lat = double.tryParse("${location["lat"]}") ?? double.tryParse("$initialLat");
        var lng = double.tryParse("${location["lng"]}") ??double.tryParse("$initialLong");
        return [lat!, lng!];
    }
  }
}
  ...
  return [double.tryParse("$initialLat") ?? 0.0, double.tryParse("$initialLong") ?? 0.0  ];
 }
}
